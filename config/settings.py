"""应用配置：环境变量 > .env.<ENVIRONMENT> 文件 > 代码默认值。

本地开发：ENVIRONMENT 默认 dev，读取 .env.dev。
测试/生产：由部署平台注入真实环境变量即可，无需携带 .env 文件。
...
class Environment(str, Enum):
    """运行环境枚举：用 Environment.PROD 而非字符串 "prod"，杜绝拼写错误。"""
...
class Settings(BaseSettings):
    ...
    DATABASE_NAME: str = "ecommerce_dev"
    ...
    def _fail_fast_on_prod(self) -> "Settings":
        ...
        if not self.DATABASE_PASSWORD or self.DATABASE_PASSWORD in ("root123", "admin123"):
